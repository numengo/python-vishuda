...
Why does this file exist, and why not put this in __main__?
  ...
  - When you run `python -mvishuda` python will execute
    ``__main__.py`` as a script. That means there won't be any
    ``vishuda.__main__`` in ``sys.modules``.
  - When you import __main__ it will get executed again (as a module) because
    there's no ``vishuda.__main__`` in ``sys.modules``.
Also see (1) from http://click.pocoo.org/5/setuptools/#setuptools-integration


The command line interface loads subcommands dynamically from a plugin folder and other things.

All the commands are implemented as plugins in the `vishuda.commands` package.
...
# PROTECTED REGION ID(vishuda.cli) ENABLED START

CONTEXT_SETTINGS = dict(auto_envvar_prefix="VISHUDA")
